...
class Sqlmanup():
    ...
    def fusion(self, SrcTableName, DestTableName):
        if self._inspector.has_table(SrcTableName) and self._inspector.has_table(DestTableName):
            srcTable = sqlalchemy.Table(SrcTableName, self._metadata, autoload=True, autoload_with=self._engine)
            destTable = sqlalchemy.Table(DestTableName, self._metadata, autoload=True, autoload_with=self._engine)
            srcTableColumns = {column.name: column.type for column in srcTable.columns}
            destTableColumns = {column.name: column.type for column in destTable.columns}
            columns = {}
            print("Checking the matching columns")
            for columnName, srcColumnType in srcTableColumns.items():
                if columnName in destTableColumns:
                    destColumnType = destTableColumns[columnName]
                    if str(srcColumnType) == str(destColumnType):
                        columns[columnName] = srcColumnType
            if columns.__len__() < 1:
                raise Exception("No matching columns found to merge!")
            ...
            def addMissingColumns():
                ...
                for columnName, srcColumnType in srcTableColumns.items():
                    if not columnName in columns:
                        missingColumns.append((columnName, srcColumnType))
                if len(missingColumns) > 0:
                    print("adding missing Columns...")
                    with self._engine.connect() as connection:
                        for columnName, columnType in missingColumns:
                            query = f"ALTER TABLE {DestTableName} ADD COLUMN {columnName} {columnType}"
                            connection.execute(sqlalchemy.text(query))
                            print("Commitig the DataLoaded!")
                            connection.commit()
            ...
            if self._inspector.has_table(DestTableName):
                raise Exception(f"{SrcTableName} not Found!")
            raise Exception(f"{SrcTableName} and {DestTableName} not Found!")
